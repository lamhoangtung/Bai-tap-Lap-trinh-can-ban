...
int main(){
  ...
  for (int i=0;i<n;i++){
    ...
    scanf("%f",a+i);
    while (*(a+i)<1||*(a+i)>100){
      printf("Diem thi vua nhap khong hop le, vui long nhap lai diem thi cua sinh vien thu %i: ",i+1);
      scanf("%f",a+i);
    }
  }
  ...
  for (int i=0;i<n;i++){
    if (*(a+i)>=90) xuatsac++;
    if (*(a+i)>=80&&*(a+i)<90) gioi++;
    if (*(a+i)>=70&&*(a+i)<80) kha++;
    if (*(a+i)>=50&&*(a+i)<70) trungbinh++;
    if (*(a+i)<50) khongdat++;
  }
